...
impl Ignore {
    ...
    fn matched<'a, P: AsRef<Path>>(
        ...
        if !self.0.overrides.is_empty() {
            let mat = self.0.overrides.matched(path, is_dir).map(IgnoreMatch::overrides);
            if !mat.is_none() {
                return mat;
            }
        }
    ...
    fn matched_ignore<'a>(&'a self, path: &Path, is_dir: bool) -> Match<IgnoreMatch<'a>> {
        ...
        for ig in self.parents().take_while(|ig| !ig.0.is_absolute_parent) {
            ...
            if m_ignore.is_none() {
                m_ignore =
                    ig.0.ignore_matcher.matched(path, is_dir).map(IgnoreMatch::gitignore);
            }
        ...
        if self.0.opts.parents {
            if let Some(abs_parent_path) = self.absolute_base() {
                ...
                println!("\ndirpath: {dirpath:?}; path_prefix: {path_prefix:?}");
...
impl IgnoreBuilder {
    ...
    pub(crate) fn build(&self) -> Ignore {
        let git_global_matcher = if !self.opts.git_global {
            ...
            builder.case_insensitive(self.opts.ignore_case_insensitive).unwrap();
...
mod tests {
    ...
    fn explicit_ignore() {
        ...
        assert!(err.is_none());
        let (ig, err) = IgnoreBuilder::new().add_ignore(gi).build().add_child(td.path());
        assert!(err.is_none());
    ...
    fn gitignore_allowed_no_git() {
        ...
        let (ig, err) =
            IgnoreBuilder::new().require_git(false).build().add_child(td.path());
        assert!(err.is_none());
        assert!(ig.matched("foo", false).is_ignore());
        assert!(ig.matched("bar", false).is_whitelist());
        assert!(ig.matched("baz", false).is_none());
